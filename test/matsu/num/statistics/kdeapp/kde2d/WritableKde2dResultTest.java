...
final class WritableKde2dResultTest {
    ...
    public static class 結果出力のフォーマットに関する_ラベル無し {
        ...
        public void before_結果を準備() {
            result = new GaussianStandardKde2dCalculator()
                    .calc(new double[][] { { 0d, 1d }, { 3d, 2d } });
        }

        @Before
        public void before_フォーマッターの準備() {
            // カンマ区切り, ラベルなし
            formatter = new WritingFormatter.LongTypeBuilder(separator)
                    .disableLabel()
                    .build();
        }
    ...
    public static class 結果出力のフォーマットに関する_ラベル有り_String指定 {
        ...
        public void test_結果の検証() {
            ...
            // 最初の行はヘッダ
            assertThat(ite.next().startsWith(labelHeader), is(true));
    ...
    private static String[] resultToStrings(
            WritableKde2dResult result, WritingFormatter parameter) {
